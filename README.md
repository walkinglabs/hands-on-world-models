...
完整课程总纲见 [`docs/课程总纲.md`](docs/课程总纲.md)。下面展开全部 65 个正文 Page，不用章节代号或实验代号代替知识内容。

### 第一部分：引言

课程先用可以逐格检查的九格世界建立最小闭环。读者要先分清观察、状态、动作、动力学、规划器与策略，再进入神经网络。

#### [第 1 章　为什么需要世界模型](docs/chapters/01-why-world-models/02-observation-and-prediction.md)
...
2. **1.2 什么是世界模型**
3. **1.3 经典世界模型**
4. **1.4 动手：在想象中驾驶（交互式体验）**：纯体验，不写代码，用震撼的 Demo 勾起读者的兴趣。
...
4. **4.4 想象训练（Dreamer）**：在 latent rollout 中训练价值函数和策略。
...
2. **5.2 图像与视频词元化（Tokenizer）**：比较像素、连续 latent、离散 token 和特征目标。
3. **5.3 动作条件注入**：理解序列建模、扩散生成和 Diffusion Forcing 的分工。
4. **5.4 键值缓存（KV Cache）与实时生成**：讨论长时一致性、重访一致性、KV 缓存、蒸馏和延迟。
...
3. **6.3 目标网络（EMA）**：从图像掩码扩展到时空块和视频表征。
...
8. **7.8 动手：把世界模型接上身体（毕业设计）**：融合视觉、关节与触觉信息，预测滑移和接触稳定性。
...
2. **8.2 鸟瞰图（BEV）与三维占用网格**：把多相机观察变成统一空间表示。
3. **8.3 神经辐射场（NeRF）与三维高斯**：比较 NeRF、3DGS 与显式几何。
