...
- **[2026-08-13]** 🎉 发布课程初版，并确定九章、60 个正文 Page、三个附录的重构总纲。
...
- [x] 确定九个大章、60 个正文 Page、三个附录与五条选修路线
- [x] 发布 1.6–3.4、路线 Notebook、PA0–PA2 与统一评价
...
完整课程总纲见 [`docs/课程总纲.md`](docs/课程总纲.md)。下面展开全部 60 个正文 Page，不用章节代号或实验代号代替知识内容。
...
1. **1.1 观察、状态与历史**：一帧观察遗漏了什么，历史为什么能够恢复隐藏状态。
2. **1.2 动作条件预测**：从“未来会怎样”走到“执行这个动作后会怎样”。
3. **1.3 多步推演与规划**：区分单步预测、rollout、规划器和策略。
4. **1.4 从经历学习动力学**：用轨迹估计转移规律，并认识模型误差。
5. **1.5 观察与动作的联合分布**：把策略、视频生成、逆动力学和可控预测看成同一联合分布的不同查询。
6. **1.6 世界模型的定义**：划清世界模型、被动视频生成器、物理引擎和无模型策略的边界。
7. **1.7 经典世界模型（World Models）**：用 V–M–C 理解表示、记忆和控制器怎样形成闭环。
8. **1.8 动手：九格世界的从零实现**：手写转移、rollout 与规划器，观察规划深度和模型漏洞。
...
1. **2.1 张量、时间与轨迹**：读懂批次、时间、通道与空间维度，正确对齐观察和动作。
2. **2.2 图像编码器**：比较卷积网络与视觉 Transformer 的表示方式。
3. **2.3 记忆与动力学**：从 RNN、GRU、Transformer 走到确定性与随机隐状态。
4. **2.4 压缩与生成**：理解 VAE、VQ-VAE、离散 token 与扩散模型的基本接口。
5. **2.5 空间表示**：认识相机、点云、鸟瞰图和占用网格。
6. **2.6 决策接口**：理解价值、策略、规划器怎样使用模型预测。
...
1. **3.1 经历与转移**：定义 episode、transition、终止状态和时间对齐。
2. **3.2 经验回放与数据切分**：按 episode 切分数据，避免相邻帧与未来信息泄漏。
3. **3.3 从经历学出转移模型**：从计数模型理解 \(p(s_{t+1}\mid s_t,a_t)\)。
4. **3.4 世界模型的基本检查**：比较弱基线、多步漂移、反事实动作、未见组合与规划增益。
5. **3.5 动手：表格世界模型的从零开始实现**：从轨迹学习转移表，再用模型预测控制完成任务。
...
1. **4.1 潜在状态世界模型**：把高维观察压缩成适合预测和决策的状态。
2. **4.2 循环状态空间模型（RSSM）**：用确定性记忆和随机状态表达历史与不确定性。
3. **4.3 交叉熵方法与模型预测控制**：用 CEM、PlaNet 与 TD-MPC2 比较候选动作序列。
4. **4.4 Dreamer 与想象训练**：在 latent rollout 中训练价值函数和策略。
5. **4.5 蒙特卡洛树搜索（MuZero）**：只学习搜索所需的表示、奖励、价值与策略。
...
1. **5.1 视频世界模型**：区分被动视频生成与动作条件世界模型。
2. **5.2 词元化与预测目标**：比较像素、连续 latent、离散 token 和特征目标。
3. **5.3 自回归与扩散**：理解序列建模、扩散生成和 Diffusion Forcing 的分工。
4. **5.4 动作条件与可控性**：比较拼接、FiLM、AdaLN 与交叉注意力。
...
1. **6.1 特征预测**：理解不重建像素的预测目标。
2. **6.2 掩码、EMA 与表示坍缩**：学习上下文编码器、目标编码器和防坍缩机制。
3. **6.3 视频 JEPA**：从图像掩码扩展到时空块和视频表征。
...
1. **7.1 机器人学习接口**：统一描述本体、观察、动作、控制频率、URDF/MJCF/USD 与数据格式。
2. **7.2 模仿学习与生成策略**：解释行为克隆漂移、动作多模态、动作分块、扩散/流匹配与 RTC。
3. **7.3 视觉语言动作模型（VLA）**：比较视觉编码器、语言条件、动作专家与生成头。
4. **7.4 机器人世界模型**：把世界模型用作策略检查器、训练辅助模型和数据引擎。
5. **7.5 操作、接触与触觉**：连接机械臂、灵巧手、滑移预测、接触稳定性与高频控制。
6. **7.6 腿式与全身控制**：理解任务分解、运动先验、参考跟踪、平衡与移动操作。
7. **7.7 模拟器与 Sim-to-Real**：使用 Isaac Lab、MuJoCo、ManiSkill、域随机化、蒸馏和系统辨识。
8. **7.8 动手：从零实现 VLA 与世界模型检查器**：训练小型 VLA，并在执行前比较候选动作后果。
9. **7.9 动手：机械臂的仿真与真机迁移**：从抓取到插接，检查视觉、摩擦、延迟和接触差异。
...
1. **8.1 相机几何与投影**：从内参、外参与深度恢复三维点。
2. **8.2 鸟瞰图与占用网格**：把多相机观察变成统一空间表示。
3. **8.3 神经辐射场与三维高斯**：比较 NeRF、3DGS 与显式几何。
4. **8.4 四维场景**：让三维表示随时间、对象和动作变化。
5. **8.5 驾驶世界模型**：预测未来视频、鸟瞰图与占用，并服务闭环驾驶。
6. **8.6 物理先验**：比较软约束、硬结构、模拟器与残差学习。
7. **8.7 动手：三维重建与占用预测**：完成相机反投影、动态场与 future occupancy 实验。
...
1. **9.1 感知质量与功能效用**：区分“看起来真实”与“足以支持决策”。
2. **9.2 动手：世界模型的系统评测**：完成多步、反事实、分布外、校准与规划器漏洞测试。
3. **9.3 动手：从失效到下一台世界模型**：把稳定失效变成竞争假设、最小改动与可证伪实验。

### 附录：随学随查的工具箱

#### [附录 A　数学、代码与术语速查](docs/appendices/math-code-glossary.md)

概率与期望、梯度与链式法则、KL 与熵、常用 PyTorch 片段、PyTorch–JAX/Flax 接口和中英文术语。

#### [附录 B　数据、算力与交付标准](docs/appendices/data-compute-delivery.md)

数据切分与泄漏检查、数据可用状态、smoke/course/research 算力配置、实验复现字段和课程交付规范。

#### [附录 C　论文、榜单与产业地图](docs/appendices/papers-benchmarks-industry.md)
...
**第一次系统学习**：按顺序完成第 1–3 章与 PA0。第 1 章提出问题，第 2 章认识组件，第 3 章把连续经历整理成数据、学出第一台模型。

**然后选择一条路线**：决策与规划、互动视频、JEPA、VLA 与机器人、空间世界——选哪一条取决于你想让模型输出什么，而不取决于侧栏里的先后位置。完成路线 Notebook 与 PA1 后，再进入第 9 章并完成 PA2。
...
```bash
jupyter lab notebooks/01_reinvent/F0-invent-a-world-model.ipynb
```
