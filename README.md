...
最终规模为 **9 章、67 个正文 Page、3 个附录**。学生的实际学习路径约为：共同基础 22 页、一条设计路线 6–12 页、评测与研究设计 4 页。

完整课程总纲见 [`docs/课程总纲.md`](docs/课程总纲.md)。下面展开全部 67 个正文 Page，不用章节代号或实验代号代替知识内容。
...
6. **3.6 动手：重新发明一台可学习世界模型**：换一张地图，自己留一个缺口，让一种失败稳定出现。
...
8. **4.8 动手：Dreamer 的完整闭环**：从环境收集、在想象中训练、再回到环境，交出一条有证据的闭环。
...
7. **5.7 动手：听从按键的视频小世界**：把 tokenizer 训到能辨认物体，并证明画面真的听从按键。
...
6. **6.6 动手：审问一个视频 JEPA**：排查表示坍缩，用探针证明特征里留下了位置与运动。
...
12. **7.12 动手：VLA 与动作后果检查**：训练一台能听指令的策略，再让后果模型在执行前拦下会撞的动作。
...
8. **8.8 动手：空间世界二选一**：在四维神经场与驾驶未来占用之间选一条，写清结论的边界。
...
4. **9.4 动手：设计下一台世界模型**：固定一个稳定失效，只改一件事，设计能证明自己错的实验。
