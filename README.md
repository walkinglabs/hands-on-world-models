...
- [x] 发布 0.6–2.4、路线 Notebook、PA0–PA2 和统一评价；
...
全书共四部分、九个大章。每个大章有一页导读，下面再分成若干可以独立阅读的正式小章。第一部分用小环境建立世界模型的基本问题；第二部分接起常用组件、数据和第一台可学习模型；第三部分提供五条可以独立选择的路线；第四部分讨论评价和研究设计。附录提供数据、运行证据和教学参考。
...
第 0–2 章完成后，从下面五章中选择一章继续学习。侧栏按 3–7 排列，彼此不是先修关系。
...
完成共同基础后，从第 3–7 章选择一条路线：决策与规划、互动视频、JEPA、VLA 与机器人、空间世界。选择哪一条取决于你想让模型输出什么，而不是取决于侧栏中的前后位置。完成路线 Notebook 和 PA1 后，再学习第 8 章并完成 PA2。
