...
传统的物理引擎（如早期的 PhysX 或 Havok）最初为电子游戏设计，它们倾向于牺牲物理定律的严格性来换取视觉上的夸张效果和计算速度。然而，当我们试图在仿真中训练控制策略，并期望将其无缝部署到真实的机械臂或四足机器人上时（即 Sim-to-Real 迁移），任何微小的动力学误差都会被闭环控制系统指数级地放大。为了解决这一痛点，Emanuel Todorov 等人在 2012 年提出了 MuJoCo (Multi-Joint dynamics with Contact) [[Todorov et al., 2012]](https://doi.org/10.1109/IROS.2012.6386109)。MuJoCo 摒弃了游戏引擎常用的极大坐标法（Maximal Coordinates），转而采用了广义坐标（Generalized Coordinates），并且创新性地将接触（Contact）和摩擦力（Friction）的计算建模为一个凸优化问题。
...
*   过去传统的商业物理引擎往往倾向于为了浮夸的视觉效果而牺牲动力学计算的精度。而基于严密理论推演的 [[Todorov et al., 2012]](https://doi.org/10.1109/IROS.2012.6386109) 研究所提出的 MuJoCo，无可辩驳地确立了广义坐标法（Generalized Coordinates）与凸优化接触求解相结合的数学新标准，使其成为现代具身智能与强化学习底层建设的绝对基石。
