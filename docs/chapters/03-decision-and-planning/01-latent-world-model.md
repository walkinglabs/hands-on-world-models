...
>
> 👉 本章实验：[动手：潜在状态世界模型（RSSM）](/chapters/03-decision-and-planning/07-decision-and-planning)（A1）、[在想象中规划与行动（PlaNet、Dreamer、MuZero）](/chapters/03-decision-and-planning/07-decision-and-planning)（A2）
...
下一篇我们拆开 $s_t$，看 RSSM 怎样把「长期记忆」和「当前不确定性」分开存放。动手实现见 [A1：潜在状态世界模型（RSSM）](/chapters/03-decision-and-planning/07-decision-and-planning)。
