...
- $\alpha_t \in [0, 1]$ 为类别平衡权重。

**初等代数直觉**：
- 当面对极其容易判断的空气体素时，网络预测概率极高（如 $p_t = 0.99$），调制因子 $(1 - 0.99)^2 = 0.0001$，使其损失被压低了 **10000 倍**，几乎不产生梯度干扰；
- 当面对罕见的障碍物时，网络往往信心不足（如 $p_t = 0.2$），调制因子 $(1 - 0.2)^2 = 0.64$，其损失被重点放大，迫使网络把全部注意力集中在攻克长尾障碍物上！
