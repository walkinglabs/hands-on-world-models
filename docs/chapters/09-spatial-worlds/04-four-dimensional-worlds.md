# 4D 时空世界模型
:label:sec_4d_world_models

在深度学习的早期探索中，无论是卷积神经网络（CNN）还是循环神经网络（RNN），我们往往倾向于对低维或者被严重投影降维后的数据进行建模。例如，将丰富的三维物理世界压缩为二维图像的序列，或是将动作与状态映射到一维的隐向量中。然而，这种处理方式在具身智能（Embodied AI）、自动驾驶以及复杂物理环境模拟等任务中，暴露出巨大的局限性。最初提出“世界模型”概念的经典文献 `[Ha & Schmidhuber, 2018]` 在二维像素隐空间中取得了巨大成功，但当智能体面临快速的视角切换、严重的三维几何遮挡以及复杂的动态交互时，基于 2D 隐空间的模型常常会预测出违反基础物理法则的“幻觉”。

为突破这一瓶颈，研究者们将空间维度的三维表征（如神经辐射场 NeRF、3D 高斯溅射点云 Gaussian Splatting、体素网格 Voxel Grids）与时间维度的序列动力学结合，提出了 **4D 时空世界模型** `[Hu et al., 2023; Blattmann et al., 2023]`。这类模型强制要求神经网络不仅要掌握“场景在当前时刻的三维几何结构是什么样的”，更要学会“在物理定律和动作的驱动下，这组三维结构在未来时刻将如何演化与形变”。

## 从运动学场到四维张量表征
:label:subsec_4d_kinematics
...
:eqlabel:eq_4d_continuous_field
...
:eqlabel:eq_4d_marginal_likelihood
...
让我们温柔地拆解公式 :eqref:eq_4d_elbo 中蕴含的物理意义：
...
## 总结
:label:subsec_4d_summary

4D 时空世界模型代表着人工智能向理解真实物理世界迈出的坚实一步。通过强制在隐空间中维持显式或半显式的三维结构（公式 :eqref:eq_4d_elbo 中的空间渲染项），并运用变分推断约束其在时间轴上的演化（时间动力学正则项），这类模型成功地抑制了因过度降维而引发的物理规律破坏。在自动驾驶和泛化机器人控制等对安全性与几何精准度要求极高的领域，4D 架构正在逐渐确立其核心范式的地位。

## 练习

1. 在公式 :eqref:eq_4d_elbo 中，如果我们假设系统具有绝对的确定性演化（即给定 $\mathbf{s}_{t-1}$ 和动作 $\mathbf{a}_{t-1}$ 后，$\mathbf{s}_t$ 唯一确定），第二项 KL 散度会发生什么变化？这对我们训练世界模型的梯度反向传播会带来什么技术挑战？
   *提示：思考连续型概率密度函数在退化为确定性分布（如狄拉克 $\delta$ 函数）时的散度性质，以及为什么研究者必须引入重参数化技巧（Reparameterization Trick）。*
2. 观察本节的代码实现，为什么我们在处理时间注意力时，需要将张量重新排列为 `(B * D * H * W, T, C)` 的形状？
   *提示：思考在这个操作中，注意力的查询（Query）和键（Key）分别代表了什么物理实体。这样排列是否意味着空间中不同坐标点（如左上角像素和右下角像素）在同一个注意力头内部是相互隔离的？如果是，在复杂物理模拟中这样做合理吗？*
3. 假设我们要将这个 4D 世界模型应用于无人机的控制系统，其直接输入是无人机下方携带的单目摄像头的 2D 视频流。在缺失多视角摄像头的情况下，模型仅凭公式 :eqref:eq_4d_elbo 能否自动学习到正确的三维结构？
   *提示：回顾几何学中的多视图几何基础（Multi-view Geometry）。单目视频序列在引入机体自身的运动位姿（如 IMU 数据或动作序列）后，能否在时间轴上等价于静态环境下的多视角观测？*
