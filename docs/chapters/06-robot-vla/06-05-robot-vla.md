...
这一节规模打折，原理不打折。先搭一台会听指令的 Tiny VLA，再在动作执行前加一个后果检查器。空间几何和驾驶占用留给 [7.6](/chapters/07-spatial-worlds/07-06-spatial-world)。
...
完成后进入 [PA1-D · 动手：Tiny VLA 与 World-Model Checker](/assignments/pa1-d)。空间几何见 [7.6 动手：空间世界实验](/chapters/07-spatial-worlds/07-06-spatial-world)。
