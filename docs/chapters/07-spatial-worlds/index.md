...
1. [6.1 相机几何与投影](./07-01-camera-geometry.md)：针孔模型、内外参，以及为什么一个像素只是一条射线。
2. [6.2 BEV、占用网格与 LSS](./07-02-bev-and-occupancy.md)：点云、鸟瞰图、占用体素，以及 Lift-Splat-Shoot 怎样把图像放进 BEV。
3. [6.3 NeRF、3DGS 与网格](./07-03-nerf-3dgs-mesh.md)：辐射场、体渲染方程、高斯泼洒和三角面片，三种静态三维表示的取舍。
4. [6.4 四维世界（4D）](./07-04-four-dimensional-worlds.md)：规范场加变形场、动作条件的 4D，以及会动与会随指定动作动的区别。
5. [6.5 驾驶世界模型与未来占用](./07-05-driving-world-models.md)：为什么预测的是未来占用而不是视频，以及开环与闭环的边界。
