...
这条路线暂时不追求高清画面。若目标是让人看见可交互视频，第 3 章会使用另一种输出与评价。
...
这些失败不应该被最好的一条 return 曲线遮住。PA1-A 至少提交一组固定 seed 的模型漏洞，并在第 7 章判断应该补数据、改状态、改目标还是限制规划。
