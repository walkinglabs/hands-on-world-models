...
- 朴素自回归解码的**时间复杂度呈二次方增长**，使得大规模视频生成推理举步维艰。
- 借由自注意力机制内在的线性组合与解耦性质，我们严格推导出**键值缓存（KV Cache）**不仅是计算上的近似，而是数学上精确等效的计算优化，大幅压低了自回归生成的计算时延。
- 视频数据的三维特性导致时空词元激增，随之而来的显存墙问题进一步催生了类似操作系统管理内存的**分页注意力（PagedAttention）**技术。这对于深入理解与部署前沿的实时视频生成模型至关重要。
