...
> ```
>
> 动手实验：[5.6 动手：动作条件视频模型的从零实现](/chapters/05-interactive-video/05-interactive-video-scratch)（两份 Notebook 涵盖离散 Token + 动作自回归与扩散去噪比较）。
...
自回归模型把联合概率拆成一连串条件预测：历史 → 下一 token 或下一帧 → 放回历史 → 再预测下一步。离散 AR 使用交叉熵预测 token；连续 AR 也可以直接回归下一帧或连续 latent。因果遮罩保证模型不能偷看未来。部署时使用 KV Cache 保存已经计算过的历史。

AR 的优点是因果接口清楚，适合流式生成。问题也很直接：训练时看到真实历史，生成时不断看到自己的预测。一步错误会变成下一步输入，这就是 train–inference gap。

### Diffusion：把一段带噪视频慢慢还原

Diffusion 可以同时给整段未来加噪，再让模型读取历史、动作和噪声等级，把它逐步还原。它能表达同一动作下的多种合理画面，训练也像普通监督回归一样稳定；但传统做法需要多次去噪，互动延迟较高。一次生成整段 clip 还可能使用片段内部的双向信息，不适合直接当作无限流式模型。

### AR-Diffusion 与 Diffusion Forcing

AR-Diffusion 在时间上自回归、在每个生成块内去噪：先用历史决定下一帧，再通过若干次去噪生成这一帧。Diffusion Forcing 允许每个时间位置拥有自己的噪声等级——历史帧可以接近干净，较远未来可以更嘈杂——从而用同一个模型切换 next-frame、整段生成与补全。
...
| 只关心规划所需特征                     | 不一定生成视频，可转向第 4 或第 6 章 |
