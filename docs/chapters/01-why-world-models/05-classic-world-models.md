...
这里 $\pi_k$ 是各分量的权重。RNN 保存历史，混合密度输出描述随机结果——这正是第 1.2 节说的"保留多峰分布"。
...
这一步对应第 1.3 节的分工：世界模型 $f$ 产出未来，规划 / 策略使用未来。这里的"Controller"还很简单，真正的规划器要在 PlaNet 和 Dreamer 中才出现。
