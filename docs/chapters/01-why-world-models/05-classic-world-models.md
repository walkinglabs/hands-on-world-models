...
世界模型不是某种单一神经网络的专有名称，而是一套将感知压缩、记忆推演与决策行动有机融合的系统化设计范式。2018 年，David Ha 与 Jürgen Schmidhuber 发表划时代论文《World Models》，首次确立了视觉（Vision, V）、记忆（Memory, M）与控制（Controller, C）的三位一体经典架构。本节我们将沿着世界模型的演进脉络，从经典的 V-M-C 范式深入到 PlaNet 的 RSSM 状态空间、Dreamer 的梦境策略学习以及 MuZero 的纯潜空间规划，建立起贯通经典与前沿的完整架构图谱。
...
- **PlaNet 与 Dreamer** 通过 RSSM 状态空间与可微想象梯度，将世界模型推向了端到端高效学习的黄金时代。
- **MuZero** 证明了不重建像素也能在纯任务潜空间中完成超人水平的树搜索规划。

理论与架构的推演至此全部打通！下一篇 [1.6 动手：从零重新发明世界模型](/chapters/01-why-world-models/06-invent-a-world-model) 将带领大家进入本章的综合代码实战：不依赖任何复杂的第三方深度学习框架，完全用纯 Python 语法把我们讲过的“观察-状态-预测-推演-学习-MPC”六大核心环节全手工实现并亲眼见证闭环运行！
