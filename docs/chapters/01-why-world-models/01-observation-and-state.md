...
本章作为全书的开篇总纲，旨在彻底理清“人工智能为何必须拥有世界模型”的根本认知逻辑。全章由一条从**物理直觉 $\to$ 数学形式化 $\to$ 历史脉络 $\to$ 动手实操**的层层递进脉络构成：

```mermaid
flowchart TD
    A["1.1 观测与状态<br/>(单帧物理观测不全，历史聚合为马尔可夫状态)"] -->|"明确状态定义后"| B["1.2 什么是世界模型<br/>(状态转移、奖励与物理规则形式化定义)"]
    B -->|"站在巨人肩膀上"| C["1.3 经典世界模型演进<br/>(从控制论、卡尔曼滤波到当代神经世界模型)"]
    C -->|"理论走向实操体验"| D["1.4 体验世界模型：交互式驾驶<br/>(观察-预测-选择-验证四步闭环实战)"]
...
### 本章递进逻辑深度拆解：
1. **1.1 节（根源追溯）**：从经典力学运动学出发，剖析为什么“单帧像素不是状态”，推导出必须聚合历史构建马尔可夫状态表示；
2. **1.2 节（形式化定义）**：给出世界模型三大核心数学接口（状态转移模型、观测预测模型、奖励价值模型），确立全书研究对象；
3. **1.3 节（历史演进八十年）**：梳理从控制论内模原理、贝叶斯滤波到现代生成式神经世界模型的演化谱系；
4. **1.4 节（动手实战）**：在交互式小车模拟环境中，亲身体验“观察—前向推演—选择动作—环境反馈”的世界模型控制闭环！
...
为了让抽象的数学概念落地，让我们先假设一个最简单的经典物理力学场景：一辆在直线上行驶的汽车。
