...
在正式定义世界模型之前，我们必须首先理解其学术渊源。在强化学习领域，使用模型来辅助决策的思想可以追溯到 Richard Sutton 在 1990 年提出的 Dyna 架构 [[Sutton, 1990]](https://dl.acm.org/doi/10.5555/645530.658292)。Dyna 架构首次明确地将强化学习分为两个过程：从真实环境交互中学习，以及从内部模型生成的模拟经验中学习（即规划）。

早期许多模型式方法在离散、低维状态空间中验证。2018 年，David Ha 和 Jürgen Schmidhuber 的 *World Models* 将高维视觉输入压缩为低维隐变量，并用循环网络预测隐变量与终止信号 [[Ha & Schmidhuber, 2018]](https://arxiv.org/abs/1803.10122)。论文还展示了一个更具体的结果：在 VizDoom 实验中，控制器可以只在学到的模型生成的轨迹中训练，再直接放回真实游戏环境测试；这证明了“在模型中训练”可行，但并不等于对任意环境都能零样本迁移。
...
仔细观察该公式，你会发现一个深刻的规律：**系统未来的状态 $s_{t+1}$，完全且唯一地由当前状态 $s_t$ 和当前采取的动作 $a_t$ 所决定。**
...
1. 在该公式中，我们假设了转移概率服从多元高斯分布。如果环境中存在高度多模态（Multi-modal）的不确定性（例如：在岔路口，你要么向左转，要么向右转，但绝对不可能直行撞墙），单一的高斯分布还能很好地建模这种动态吗？为什么？
