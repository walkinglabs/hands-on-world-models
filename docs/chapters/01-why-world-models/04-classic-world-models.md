...
理论与架构的推演至此全部打通。回到开篇的 [1.1 动手：九格世界的从零实现](/chapters/01-why-world-models/01-invent-a-world-model) 亲手完成本章的综合代码实战：不依赖任何第三方深度学习框架，只用 Python 标准库把“观察—状态—预测—推演—学习—MPC”六个环节全部手工实现，并亲眼看到闭环跑起来。
