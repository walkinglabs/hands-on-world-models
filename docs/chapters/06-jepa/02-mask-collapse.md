...
:label:sec_mask_collapse

在深度学习的发展历程中，自监督学习（Self-Supervised Learning, SSL）逐渐成为从未标注数据中提取通用表征的核心范式。自监督学习的精髓在于“从数据中挖掘监督信号”，即利用输入数据自身的一部分去预测另一部分。早期的经典工作，如自然语言处理领域的 BERT [Devlin et al., 2018] 以及计算机视觉领域的掩码自编码器（Masked Autoencoders, MAE）[He et al., 2021]，均在原始输入空间（如词元或像素空间）中进行掩码预测，并取得了举世瞩目的成功。
...
为了引导模型关注高阶的抽象语义，研究者们提出跳过像素解码，直接在抽象的潜在空间（Latent Space）中进行预测。诸如联合嵌入预测架构（Joint-Embedding Predictive Architecture, JEPA）[Baevski et al., 2022; Assran et al., 2023] 的提出，正是这一理念的巅峰体现。但是，当我们放弃了由原始像素提供的绝对真实标签（Ground Truth），转而让模型去预测自己动态生成的潜在特征时，一个致命的优化陷阱便悄然浮现——**表征坍塌（Representation Collapse）**。

本节我们将从最基础的代数与几何视角，极其严谨地剖析表征坍塌的数学本质。我们将探讨为什么基于梯度的优化器会不可避免地滑向这个陷阱，并详细推导现代自监督学习如何通过非对称架构与动量编码器（如 BYOL [Grill et al., 2020] 中提出的核心技术）来优雅而彻底地规避这一问题。

## 潜在空间预测与优化的“惰性”
:label:subsec_latent_prediction_laziness
...
:eqlabel:eq_collapse_scalar_loss
...
从代数的角度看，如果要最小化 :eqref:`eq_collapse_scalar_loss` 使得损失为 $0$，最简单的数学解是什么？显然，只需要让函数 $f_\theta$ 退化为一个不受输入 $x$ 影响的常数函数，即对于任意的 $x$，都有 $f_\theta(x) = c$。
...
## 从标量到高维张量的坍塌证明
:label:subsec_tensor_collapse_proof
...
:eqlabel:eq_collapse_vector_loss
...
:eqlabel:eq_collapse_vector_step1
...
:eqlabel:eq_collapse_inequality
...
要打破这种坍塌，最直接的思路是从物理学中汲取灵感：既然吸引力（使得相似样本在空间中靠近）会导致坍塌聚拢，那么我们只要引入一种排斥力即可。这正是对比学习（Contrastive Learning，如 SimCLR [Chen et al., 2020]）的核心思想。
...
注意 :eqref:`eq_sg_loss` 中，变量 $\xi$ 被视作常数，不参与关于 $\theta$ 的偏导数计算。如果 $\xi$ 完全固定不动，这就等价于传统的监督学习（用随机初始化的网络生成随机但不坍塌的伪标签）。但固定的特征空间过于贫瘠，无法学到高质量的语义。
...
## 小结
:label:subsec_mask_collapse_summary
...
## 练习
1. 推导挑战：在 :eqref:`eq_collapse_vector_step1` 中，如果网络不是简单的线性映射 $\mathbf{W}\mathbf{x}$，而是带有一个非线性层，比如 $f(\mathbf{x}) = \mathrm{ReLU}(\mathbf{W}\mathbf{x})$，此时还会发生必然的零值坍塌吗？为什么？
    * **提示**：回顾 ReLU 的定义，并思考如果所有权重初始化为较小的负数或者在优化初期趋向负数，网络的输出会受到什么影响。
2. 动量衰减系数的极端情况：在我们的代码实验中，衰减系数被设为了 $\tau = 0.99$。如果我们将动量系数设置为 $\tau = 0$（即每次迭代，目标网络直接完全复制在线网络），会发生什么？如果将其设置为 $\tau = 1.0$ 呢？尝试修改代码运行并利用数学知识给出解释。
    * **提示**：对于 $\tau = 0$，请思考此刻非对称架构是否又退化回了完全对称；对于 $\tau = 1.0$，思考目标网络的靶点是否还能捕捉到最新的语义信息。
3. 对比学习的联系：在不使用负样本和 EMA 的情况下，如果强制要求每次特征提取后的输出必须在一个超球面上（即通过 L2 范数归一化 $\mathbf{h} = \mathbf{h} / \|\mathbf{h}\|_2$），这能在某种程度上缓解特征全趋于 $0$ 的点坍塌吗？它能否阻止维度挤压在小部分子空间的“维度坍塌”？
