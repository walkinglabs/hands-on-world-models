...
>
> 动手实验：[6.5 动手：视频 JEPA 的从零实现](/chapters/06-jepa/05-jepa-scratch)（两份 Notebook 分别做视频特征预测与动作条件特征预测）。
...
[下一篇 → 6.2 掩码、EMA 与表示坍缩](./02-mask-collapse.md) · [动手：视频特征预测](/chapters/06-jepa/05-jepa-scratch)
