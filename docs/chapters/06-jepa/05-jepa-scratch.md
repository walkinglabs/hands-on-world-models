...
为了克服这些瓶颈，Yann LeCun 提出了联合嵌入预测架构（Joint-Embedding Predictive Architecture, JEPA）[LeCun, 2022]，并随后在视觉领域由 I-JEPA [Assran et al., 2023] 证明了其有效性。JEPA 的核心洞见在于：**与其在原始输入空间中预测未知的细节，不如在抽象的表征空间中预测缺失的信息**。这种架构摒弃了像素级重建，同时也无需手工构造对比学习中的负样本。
...
我们首先定义一个通用的多层感知机（MLP）块，它将承担公式 :eqref:eq_jepa_encoders 和 :eqref:eq_jepa_predictor 中非线性映射的重任。
...
现在，我们将上下文编码器、目标编码器和预测器组合成一个完整的 JEPA 模块。这里我们需要特别注意公式 :eqref:eq_jepa_ema 中的 EMA 逻辑的实现。在初始化时，目标编码器是上下文编码器的精确副本。在每次前向传播或优化后，我们需要平滑地更新目标编码器的权重。
...
在拥有了完整的架构后，我们可以编写单次迭代的训练逻辑。训练过程严格遵循我们推导的公式：计算前向传播得到预测表征 $\hat{\mathbf{S}}_y$ 和目标表征 $\mathbf{S}_y$，根据公式 :eqref:eq_jepa_loss 计算均方误差（MSE），通过反向传播仅更新 $\theta$ 和 $\phi$，最后调用公式 :eqref:eq_jepa_ema 更新 $\bar{\theta}$。
