...
要深刻理解 EMA 动量更新的物理本质，我们必须从信号处理中的**一阶低通时域滤波**与动力系统中的**非对称不动点稳定性**展开剖析。

### 1. 师生自举系统（Teacher-Student Bootstrap）
- **在线上下文编码器（学生网络 $\theta$）**：每时每刻接收批次数据的随机梯度反向传播更新，快速学习新知识，但参数波动剧烈；
- **动量目标编码器（导师网络 $\phi$）**：**彻底切断反向传播求导通道（Stop-Gradient, $\text{sg}[\cdot]$）**。导师的参数完全来自于过去成千上万个历史时刻学生网络参数的指数加权平均；
- **非对称动力学生态**：学生的目标是努力预测出平稳的导师特征。由于导师特征是一个随时间平缓漂移的外部移动目标，使得全局常数解失去吸引子性质。

### 2. EMA 动量参数更新核心方程与有效记忆窗口
在每一个训练迭代步 $t$：
...
$$\phi_t = (1 - \tau) \sum_{k=0}^{t-1} \tau^k \theta_{t-k} + \tau^t \phi_0$$

> **有效平滑时域窗口推导**：
> 权重序列 $(1 - \tau) \tau^k$ 构成了一个经典的几何级数。导师参数相当于对过去历史参数做指数衰减卷积，其**有效时间记忆步长（Effective Window Length）**满足：
> $$T_{\text{eff}} \approx \frac{1}{1 - \tau}$$
> - 当 $\tau = 0.99$ 时，有效回看窗口为 $T_{\text{eff}} \approx 100$ 步；
> - 当 $\tau = 0.996$（I-JEPA 默认值）时，有效回看窗口高达 $T_{\text{eff}} \approx 250$ 步；
> - 当 $\tau = 0.999$ 时，有效回看窗口高达 $T_{\text{eff}} \approx 1000$ 步！

<details>
<summary><b>深入探讨：从动力系统与非对称梯度场证明为什么 Stop-Gradient 能彻底防止特征坍塌</b></summary>

考虑一个极简的双网络对齐损失：
$$\mathcal{L}(\theta, \phi) = \frac{1}{2} \|f_\theta(\mathbf{x}) - f_\phi(\mathbf{y})\|_2^2$$

1. **若采用对称双向反传（无 Stop-Gradient，$\phi = \theta$）**：
   $$\nabla_\theta \mathcal{L} = (f_\theta(\mathbf{x}) - f_\theta(\mathbf{y})) \left( \nabla_\theta f_\theta(\mathbf{x}) - \nabla_\theta f_\theta(\mathbf{y}) \right)$$
   若网络输出任意常数 $f_\theta(\cdot) \equiv \mathbf{c}$，则 $f_\theta(\mathbf{x}) - f_\theta(\mathbf{y}) = \mathbf{0}$，梯度处处为 0。常数坍塌点是该可微动力系统的极小值点，网络会以最快速度滑向坍塌！

2. **若引入 Stop-Gradient（$\text{sg}[f_\phi(\mathbf{y})]$）与 EMA**：
   $$\nabla_\theta \mathcal{L} = (f_\theta(\mathbf{x}) - \underbrace{f_\phi(\mathbf{y})}_{\text{视为固定常数向量}}) \nabla_\theta f_\theta(\mathbf{x})$$
   此时，损失函数不再是对称的能量阱，而是变成了一个**回归移动目标问题**！只要导师网络初值 $f_{\phi_0}$ 具有随机初始化的非零方差，学生就必须不断学习特征提取流形去拟合导师输出，使得对称常数解不再是系统的定常态！
</details>
