...
当我们希望评估世界模型的观测模型（Observation Model）或者解码器（Decoder）重建当前环境的能力时，最直观的第一步，是衡量生成的预测图像与真实的观测图像之间的相似度。我们首先从初等阶段最基础的几何距离切入。
...
_图 10.2-3：两个一维高斯的均值差与标准差差是参数平面上的两条正交分量，W2 距离由二者共同决定。_
</div>
