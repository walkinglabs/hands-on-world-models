...
## 自回归展开中的复合误差
:label:`subsec_compounding_error`
...
### 从等比数列到误差累积
:label:`subsubsec_geometric_error`
...
:eqlabel:`eq_error_accumulation_1d`
...
:label:`subsubsec_covariate_shift`
...
:eqlabel:`eq_single_step_error_bound`
...
## 目标错配：预测与控制的鸿沟
:label:`subsec_objective_mismatch`
...
## 变分下界与表征坍塌
:label:`subsec_representation_collapse`
...
1. 在公式 :eqref:`eq_error_decomp` 的推导中，假设我们知道世界的动力学函数 $f(\cdot)$ 是 $L$-Lipschitz 的，并且 $L<1$（这是一个非常稳定、总是收敛到平衡点的系统）。在这种情况下，随着推演步数 $T \to \infty$，累积误差 $e_T$ 会趋于无穷大吗？请写出它的收敛上界。
   - *提示*：回想高中数列中等比数列无穷项求和公式，当公比的绝对值小于 1 时，求和公式是什么。
2. 考虑目标错配的问题。如果我们不使用 MSE 来训练世界模型，而是提出一种叫做“价值感知模型（Value-Aware Model）”的新方法。你能否构思出一种修改公式 :eqref:`eq_mse_loss` 的思路，使得模型在预测价值较高或对奖励影响较大的状态维度上，赋予更高的损失权重？
   - *提示*：考虑在损失函数内部乘上一个依赖于梯度 $\nabla_s V(s)$ 的权重项。
3. 运行本章提供的 `WorldModelDiagnoser` 代码中的 `detect_representation_collapse` 方法。如果你发现 `活跃隐变量维度 (Active Units)` 的数量随着训练轮数的增加而突然降为 $0$，你可以采取什么手段来干预训练以拯救模型？
   - *提示*：思考如何调节变分下界公式 :eqref:`eq_elbo_world_model` 中 KL 散度项的权重系数（如 $\beta$-VAE 的思想），或者思考如何为 KL 散度设定一个硬性下界（Free bits）。
