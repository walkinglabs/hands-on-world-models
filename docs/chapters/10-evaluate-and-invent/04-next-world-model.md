...
>
> **前置知识**：你已经跑过 [3.6 动手：重新发明一台可学习世界模型](/chapters/03-data-and-first-model/06-rl-foundation-concise)、所选路线最后一份动手，以及 [9.2 动手：世界模型的系统评测](/chapters/10-evaluate-and-invent/02-systematic-evaluation)。本节从那里找到的稳定失效出发，只改一件事。9.3 讲「为什么要这样设计实验」；本节是要交的东西。
...
## 扩展：高维空间中的想象强化学习与自我改进

如果你觉得只做碰撞检查还不够过瘾，可以挑战具身世界模型的前沿：**在想象中做 RL（Imagination RL / Self-Improving）**。

像 **RISE** (Self-Improving Robot Policy with Compositional World Model) 这类工作，把 Dreamer 的核心思想搬到了高维真实机器人上：

1. 先训一个动作条件视频世界模型（如第 5 或第 7 章产物）；
2. 真实机器人**不去碰真机**，只在世界模型生成的“想象未来视频”里执行动作；
3. 用一个进度价值模型给这些想象轨迹打分（Advantage），在想象中做 PPO 强化学习，实现策略的**自我改进**。

这把世界模型从“动作后果的被动检查器”，变成了“策略主动进化的无尽训练场”。如果你的最小改动是把策略训练引擎从真实数据换成想象轨迹，并在单卡跑通一条 PPO 闭环，你完成的将是一份极具竞争力的研究报告。
...
5. **换探索（内在动机）**：引入基于世界模型预测误差的好奇心（Prediction Error as Intrinsic Motivation），模型算错的地方就是该去探索的地方。
