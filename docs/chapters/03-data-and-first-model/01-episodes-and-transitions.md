...
本章构建世界模型与智能体决策所需的**数据组织形式与强化学习控制基础**。要让世界模型学会“物理规律”并用于“指导行动”，首先必须理解数据是如何在交互中产生的、策略是如何评估优化的，以及前向规划（MPC）是如何运作的。第 3 章由以下递进逻辑图谱完整贯穿：

```mermaid
flowchart TD
    A["3.1 经历、转移与交互<br/>(MDP 状态转移、即时奖励与累积回报定义)"] -->|"收集到的海量交互数据需要存储与切分"| B["3.2 经验回放池与优先级采样<br/>(PER 破除时序自相关，恢复 i.i.d. 训练)"]
    B -->|"基于回放数据优化动作策略"| C["3.3 价值函数与策略梯度<br/>(贝尔曼期望/最优方程 + 对数导数技巧)"]
    C -->|"跳过价值网络，直接在动力学上多步前向推演"| D["3.4 模型预测控制 (MPC) 与 CEM<br/>(滚动时域有限规划与高斯精英重拟合)"]
    D -->|"纯底层强化学习代码闭环"| E["3.5 强化学习基础从零实现<br/>(Double DQN 解决高估问题与软更新)"]
    E -->|"连续动作控制与极简封装"| F["3.6 强化学习基础简洁实现<br/>(Squashed Gaussian 策略与雅可比修正)"]
...
### 本章递进逻辑深度拆解：
1. **3.1 节（数据原点）**：将物理交互形式化为马尔可夫决策过程（MDP）转移元组 $(s, a, r, s')$ 与累积回报；
2. **3.2 节（数据管理）**：构建循环经验回放池，利用优先经验重放（PER）消除时间相关性，为世界模型训练准备平稳数据流；
3. **3.3 节（无模型优化基础）**：建立贝尔曼方程与策略梯度定理，推导智能体如何在没有显式环境模型时评估长程价值；
4. **3.4 节（模型规划桥梁）**：首次引入**模型预测控制（MPC）与交叉熵方法（CEM）**，展示当拥有环境动力学模型时，如何通过前向多步推演直接求解最优动作；
5. **3.5 & 3.6 节（从零实现与优雅封装）**：手写 Double DQN 消除价值高估，并封装连续动作空间的 Squashed Gaussian 策略，打通强化学习全部工程细节！
