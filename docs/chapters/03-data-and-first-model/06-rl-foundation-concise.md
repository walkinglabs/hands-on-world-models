# 强化学习基础模块的简洁实现
:label:sec_rl_foundation_concise

在前面几节中，我们已经探讨了如何从零开始构建强化学习的基本计算图，并深入理解了其中的梯度传播机制。然而，在实际的现代深度强化学习研究与工程实践中，我们很少会手动实现每一个底层的张量操作。随着诸如深度Q网络 (Deep Q-Network, DQN) [Mnih et al., 2013] 与近端策略优化 (Proximal Policy Optimization, PPO) [Schulman et al., 2017] 等算法的提出，强化学习的算法范式逐渐收敛为几个标准化的核心模块：环境交互、经验回放、状态价值评估以及策略近似。

强化学习的理论基础可以追溯到理查德·贝尔曼 (Richard Bellman) 在20世纪50年代提出的动态规划理论 [Bellman, 1957]，以及理查德·萨顿 (Richard Sutton) 等人发展的时序差分学习 (Temporal-Difference Learning) [Sutton, 1988]。在当时，受限于计算力和数据规模，这些方法多用于状态空间离散且有限的表格型 (Tabular) 场景。而在深度学习框架的加持下，我们得以利用神经网络的高维非线性拟合能力，将这些经典的数学迭代过程转化为可以用梯度下降优化的目标函数。
...
:eqlabel:eq_q_function

结合式 :eqref:eq_return_recursive，我们可以推导出强化学习中最著名的贝尔曼期望方程 (Bellman Expectation Equation)：

$$Q^\pi(s, a) = \mathbb{E}_{s' \sim P, a' \sim \pi} [r_t + \gamma Q^\pi(s', a') \mid S_t=s, A_t=a]$$
:eqlabel:eq_bellman_q

在深度强化学习中，我们不再使用表格来记录每一个状态-动作对的精确 $Q$ 值，而是使用一组参数为 $\theta$ 的神经网络 $Q_\theta(s, a)$ 来进行函数近似。我们的代码实现，本质上就是构建数据结构来收集式 :eqref:eq_bellman_q 中的元组 $(s_t, a_t, r_t, s_{t+1})$，并构建神经网络来拟合这个数学期望。
...
## 经验回放缓冲区：打破时序相关性
:label:subsec_replay_buffer_concise
...
为了解决这一难题，Lin在1992年首次提出了经验回放 (Experience Replay) 的概念 [Lin, 1992]，随后这一机制在DQN中被发扬光大。其核心思想是：构建一个固定容量的先进先出 (FIFO) 队列。我们将智能体与环境交互产生的每一个离散转移步骤 (Transition)——即元组 $(s_t, a_t, r_t, s_{t+1}, d_t)$（其中 $d_t$ 是布尔值，表示状态 $s_{t+1}$ 是否为终止状态）——存储在这个队列中。在训练神经网络时，我们不再使用最新收集的单条轨迹，而是从队列中均匀且随机地抽取一个小批量 (Mini-batch) 的转移元组。
...
## 基于多层感知机的策略网络与价值网络
:label:subsec_policy_value_net_concise

在拥有了稳定的数据来源（经验回放池）之后，我们接下来需要定义算法的“大脑”，即策略网络和价值网络。

正如我们在式 :eqref:eq_q_function 中所定义的，动作价值函数 $Q(s, a)$ 的输入是状态和动作，输出是一个标量期望值。在状态连续而动作空间离散（例如 $a \in \{0, 1, \dots, K-1\}$）的环境中，为了避免对每一个特定的动作单独进行前向传播，我们通常将 $Q$ 网络设计为：仅接收状态向量 $s$ 作为输入，而在输出层产生一个维度为 $K$ 的向量，其中第 $k$ 个元素对应于动作 $k$ 的 $Q$ 值。这种架构在数学上等价于建立一个从状态空间到整个动作空间的非线性映射：$\mathcal{S} \mapsto \mathbb{R}^K$。
...
1. 在式 :eqref:eq_return_recursive 中，如果折扣因子 $\gamma=0$，累积回报 $G_t$ 的物理含义是什么？它会如何影响智能体的行为决策？如果 $\gamma=1$ 呢？
   * 提示：从极度短视 (myopic) 与长远规划的角度分别进行思考，并考虑当序列无限长且 $\gamma=1$ 时数学期望是否可能发散。
2. 试想如果我们将 `ReplayBuffer` 中的均匀随机采样改为按照时间顺序的连续采样（即取出最新插入的一批数据），会对多层感知机的梯度更新造成什么数学层面上的影响？
   * 提示：考虑梯度方向在参数空间中的运动轨迹，以及损失函数曲面的曲率变化情况。
3. 请详细阅读代码块，为什么 `PolicyNetwork` 的输出层没有使用诸如 Sigmoid 或 Softmax 的激活函数进行约束？在什么环节应用这些约束在数值计算上最为稳定？
   * 提示：回顾我们在“Softmax回归的简洁实现”一节中关于对数求和指数 (LogSumExp) 技巧的讨论，以及框架提供的内置交叉熵损失函数的底层实现机制。
