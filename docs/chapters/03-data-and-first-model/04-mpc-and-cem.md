...
模型预测控制的思想可以追溯到20世纪70年代后期的工业过程控制领域 `[Richalet et al., 1978]`。当时，化工厂等复杂系统面临着严格的物理约束和多变量耦合问题，传统的PID控制器难以胜任。MPC通过在线求解一个有限时域的最优控制问题，并在每个时间步应用序列的第一个动作（即滚动优化策略，Receding Horizon Control），成功地解决了这些工业难题 `[Garcia et al., 1989]`。然而，早期的MPC高度依赖于线性系统和凸优化。

随着深度学习的崛起，研究人员开始使用高容量的神经网络来拟合极其复杂的非线性系统（例如机器人的关节动力学）。在这些非线性模型上进行精确的梯度下降优化往往会陷入局部最优，甚至由于梯度的爆炸或消失而失败。为了在非线性世界模型中进行有效的规划，研究者们引入了无梯度优化方法。交叉熵方法（CEM）最初由Rubinstein在1997年提出，用于稀有事件模拟和组合优化 `[Rubinstein, 1997]`。后来，它被证明在连续空间的黑盒优化中同样卓越 `[Botev et al., 2013]`。近年来，结合深度神经网络动力学模型和CEM的MPC算法（例如PETS `[Chua et al., 2018]` 和PlaNet `[Hafner et al., 2019]`）在基于模型的强化学习（Model-Based RL）领域取得了里程碑式的进展。
...
1. 在时间步 $t$，基于当前真实状态 $s_t$，求解优化问题 :eqref:eq_mpc_optimization，得到未来 $H$ 步的最优动作序列 $\mathbf{a}^*_{t:t+H-1}$。
...
当动力学函数 $f$ 是线性系统，且代价函数 $c$ 是二次型时（即LQR问题），我们可以通过Riccati方程精确地求出解析解。然而，在基于模型的深度强化学习中，$f$ 通常是一个深度神经网络。此时，优化问题 :eqref:eq_mpc_optimization 变得极度非凸，高维空间中布满了局部极小值。
...
将高斯分布的概率密度函数代入 :eqref:eq_cem_update_general 并对 $\mu$ 和 $\Sigma$ 求导置零，我们可以得到极其简洁优美的更新公式。对于动作序列，假设各维度独立，我们只需要更新均值和方差：
...
2. **迭代优化**（循环 $K$ 次）：
   ...
   e. **分布更新**：利用精英集，通过公式 :eqref:eq_cem_mu 和 :eqref:eq_cem_sigma 计算新的均值 $\mu_{k+1}$ 和方差 $\sigma_{k+1}^2$。
