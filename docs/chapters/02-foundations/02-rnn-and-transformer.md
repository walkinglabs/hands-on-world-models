...
> 动力学演化发生于时间序列中。本节梳理循环网络状态转移与自注意力时间建模机制。
...
>
> 核心实验在 [2.7 动手：核心组件的简洁实现](/chapters/02-foundations/05-basic-components-scratch) 中串联看见、视觉、记忆、压缩、空间与规划。学完本章后，先明确模型最需要输出的形式（潜在向量、连续画面、特征表示、机器人动作或三维占用），再从第 4–8 章选择对应的设计路线——各路线彼此独立。
...
空间观测被编码为潜在 Token 后，下一个核心问题随之浮现：如何跨越时间维度，将连续历史 Token 压缩为具有长期因果记忆的动态信念状态？在下一节 [2.3 记忆与动力学](/chapters/02-foundations/02-rnn-and-transformer) 中，我们将深入剖析 RNN、GRU、Transformer 与循环状态空间模型（RSSM）的时序建模精髓。
...
> 👉 动手实验：[动手：观察编码、记忆与压缩](/chapters/02-foundations/05-basic-components-scratch)
