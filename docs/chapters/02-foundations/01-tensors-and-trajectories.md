...
>
> 这一章建立五条路线共用的语言。每个组件只讲到足以判断它的输入、输出和用途；完整实现留到真正使用它的路线文章。
>
> 文章细分，实验不按名词拆散：2.8 接起看见、视觉、记忆与压缩，再接起空间、决策与训练，3.4 接起数据、学习与闭环检查。
>
> 学完本章后，先写下模型最需要交出的结果（latent、画面、feature、机器人动作，还是三维占用），再从第三部分选择一条路线——五条路线彼此不是先修关系。
