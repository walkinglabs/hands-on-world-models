...
> **前置知识**：你已经读过第 1 章和第 2 章，并最好刚跑完 [0.6 动手：从零重新发明世界模型](/chapters/00-why-world-models/06-invent-a-world-model)。这一节把那些零件真跑一遍。
...
<div style="text-align:center; margin:20px 0;">
  ...
  <div style="font-size:0.9em; color:var(--vp-c-text-2); margin-top:8px;">这一节要接起来的两件事：看见（一张图怎么变成特征）、走进空间（深度怎么变成格子）。从经历里学习留给 2.4。模型从未见过物理定律，它要从这些计算里自己发现规律。</div>
</div>
...
> 👉 [2.4 动手：第一台可学习世界模型](/chapters/02-data-and-first-model/04-learn-a-table-world)
...
下一步是 [2.4 动手：第一台可学习世界模型](/chapters/02-data-and-first-model/04-learn-a-table-world)，然后是 [PA0](/assignments/pa0)。那里不再给你完整转移表，你要自己留一个缺口，并让一种失败稳定出现。
...
从 0.6 的九格网格到这一节的小图和深度点，核心思想没有变：**在行动之前，先在内部预见行动的后果。** 从经历里学习留给 2.4。
