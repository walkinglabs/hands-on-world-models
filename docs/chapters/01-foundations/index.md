...
这一章建立五条路线共用的语言。组件只讲到足以判断输入、输出和用途；完整实现留到真正使用它的路线文章。
...
7. [Replay Buffer 与第一台可学习模型](./01-07-data-and-first-model.md)：从 episode 学出概率转移，并完成 PA0 前的闭环。

## 本章实验

- [F1–F3：共同基础实验](/labs/foundations)
- [PA0：第一台可学习世界](/assignments/pa0)

F1–F3 仍只有三份 Notebook。文章可以细分，实验不按名词拆散：F1 接起视觉、记忆和压缩，F2 接起空间、评价和规划，F3 接起数据、学习与闭环检查。
