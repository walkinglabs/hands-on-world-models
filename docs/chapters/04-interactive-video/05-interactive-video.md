...
> **前置知识**：你已经读过 4.1–4.4，知道 VQ 码本、STE 与动作注入。最好刚跑完 [3.6 动手复现 World Models](/chapters/03-decision-and-planning/06-reproduce-world-models) 和 [3.7 动手：决策与规划实验](/chapters/03-decision-and-planning/07-decision-and-planning)。这一节把「按键之后画面会不会跟着变」真跑一遍。
...
做完这一节再进入 [PA1-B](/assignments/pa1-b)。特征空间预测留给 [5.5](/chapters/05-jepa/05-jepa)。
...
完成后进入 [PA1-B · 动手：做出一个听从按键的视频小世界](/assignments/pa1-b)。下一章换一种预测目标：不画回像素，只预测未来特征，见 [5.5 动手：JEPA 实验](/chapters/05-jepa/05-jepa)。
