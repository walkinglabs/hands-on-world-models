...
这条路线同样只依赖第 0–1 章。ViT、EMA 和 stop-gradient 会在当前问题里重新讲，不要求先学 Dreamer 或互动视频。

## 4.2 从图片 patch 到视频 tubelet
...
Action-JEPA 需要时间对齐的 observation、action、下一 observation，最好还记录控制频率、延迟与机器人自身状态。第 5 章会继续使用这种数据。
