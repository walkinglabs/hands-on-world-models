...
1. [8.1　基线与多步评价](./01-baselines-and-horizons.md)：建立弱基线，对比单步预测与开环多步滚动的本质差异，通过horizon曲线分析误差累积特性
2. [8.2　反事实、分布外与鲁棒性](./02-counterfactual-and-ood.md)：通过控制变量法验证动作条件响应，测试模型在分布外输入下的泛化能力与不确定性校准
3. [8.3　运行证据与复现](./03-hardware-evidence.md)：规范实验记录标准，明确smoke测试、目标配置与研究配置的区别，保证结果可复现
4. [8.4　失败分析与下一台世界模型](./04-next-world-model.md)：从可复现的稳定失败出发，学习提出竞争假设、设计可证伪实验的研究方法
5. [8.5　审问世界模型](./05-interrogate-world-model.md)：把六项审问写成带公式和真实数字的判据
6. [8.6　动手：审问一台世界模型](./06-test-a-world-model.md)：对解析玩具和 PA1 模型做基线、多步、反事实与 OOD 检查
7. [8.7　动手：实现自己的世界模型](./07-next-model-proposal.md)：基于失败分析提出最小改进，实现代码并通过公平对照实验验证假设
