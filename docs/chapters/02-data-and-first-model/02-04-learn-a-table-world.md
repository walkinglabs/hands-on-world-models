...
不必先上神经网络。换到 2.4 的 LineWorld：长度 7，起点 3，终点 6，陷阱 0，每一步有 \(20\%\) 概率打滑。
...
- **12 段 PixelWorld 只用来看 shape。** 真正学动态的是后面那条会打滑的线。不要把 8 / 2 / 2 写成 2.4 的切分。
...
| 计数表、反事实、MPC | 2.3、本页 |
| 自己留一个缺口再走一遍 | [PA0](/assignments/pa0) |

如果表格方法已经解决了任务，继续换上神经网络不会加分。只有当图像、历史、不确定性或动作空间确实让表格失效时，才该进入后续路线。先找到失效点，再决定加什么。

> 👉 理论预览：[1.7 经验回放与第一个模型](/chapters/01-foundations/01-07-data-and-first-model) · 作业：[PA0](/assignments/pa0)
