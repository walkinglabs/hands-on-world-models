...
1. [2.1 一段经历怎样保存](./01-episodes-and-transitions.md)：把观察、动作、奖励和下一次观察按时间接好。
2. [2.2 Replay Buffer 怎样取出可学习的数据](./02-replay-buffer-and-splits.md)：连续采样，并避免把未来信息泄漏到测试集。
3. [2.3 从经历学出第一台模型](./03-first-learned-world.md)：用计数学习概率转移，再把预测放回行动循环。
4. [2.4 动手：第一台可学习世界模型](./04-learn-a-table-world.md)：从轨迹里数出转移表，再用 MPC 走到终点。
