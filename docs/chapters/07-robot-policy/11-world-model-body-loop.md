...
三种用法共用同一句反事实检查：固定当前观察，只换候选动作，预测必须分开。分不开，你手里的就不是世界模型，只是视频生成或行为克隆——先回 [7.3](/chapters/07-robot-policy/07-vla-rtx)。
...
- 检查器从一步 outcome model 换成**多步想象 rollout**（[7.3](/chapters/07-robot-policy/07-vla-rtx) 的转移方程接 $H$ 步）；
...
**做法**：不训策略。给世界模型一张当前图像和一个目标，预测未来若干关键点的轨迹（抓手或物体上的点）；用一个跟踪控制器（IK + PD，见 [7.1](/chapters/07-robot-policy/01-multimodal-observation) 的经典控制栈）让真实身体跟着预测的点走。预测即控制。
...
下一章把空间从机器人本体扩展到整个场景与道路，见 [8.1 相机几何与投影](/chapters/09-spatial-worlds/01-camera-geometry)。

[上一篇 7.9　动手：现实迁移的简洁实现](/chapters/08-robot-sim/07-sim2real-concise) · [下一章 → 8.1　相机几何与投影](/chapters/09-spatial-worlds/01-camera-geometry)
