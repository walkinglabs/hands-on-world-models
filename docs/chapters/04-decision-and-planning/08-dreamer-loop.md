...
> **本节目标**：把决策与规划路线（第 4 章）的跟做实验接成一条有证据的真实循环——从环境收集数据，在 RSSM 中想象，用 Actor-Critic 学习，再回到环境检查。不是复现 DreamerV3 的排行榜，而是亲眼看到「接口连通」与「策略收敛」之间的鸿沟，并诚实报告它从哪里开始裂开。

> **本节代码**：[本节模板](https://github.com/walkinglabs/hands-on-world-models/blob/main/notebooks/projects/route-template.ipynb) · [学出一个潜在世界](https://github.com/walkinglabs/hands-on-world-models/blob/main/notebooks/04_decision/learn-a-latent-world.ipynb) · [在想象中行动](https://github.com/walkinglabs/hands-on-world-models/blob/main/notebooks/04_decision/act-in-imagination.ipynb) · [`neural.py`](https://github.com/walkinglabs/hands-on-world-models/blob/main/src/hwm/neural.py) · [`control.py`](https://github.com/walkinglabs/hands-on-world-models/blob/main/src/hwm/control.py)

> **前置知识**：你已经跑过 [4.7 动手：Dreamer 的简化实现](/chapters/04-decision-and-planning/07-decision-and-planning)。第一份 Notebook「学出一个潜在世界」确认 RSSM 接口连通，第二份「在想象中行动」用位置模型证明 learned dynamics 能改善真实行动，再用一次 Actor 更新接通想象训练。本节 **不再重复那两份跟做实验**，而是把它们扩展成一次完整训练。

---

第一份 Notebook 用 4 段 PixelWorld、15 次更新证明了：shape 对、梯度通、loss 能降。它也证明了另一件事——复制上一帧的像素 MSE 是 `0.0061`，15 步之后的重建仍是 `0.0144`。loss 下降不等于看见了世界。

第二份 Notebook 用 180 条单步位置转移证明了另一件事：`PositionDynamics` 的 MSE 从 `0.315` 降到 `0.0001` 之后，四个不在训练网格上的起点，规划成功率 `1.0`，随机是 `0.0`，平均剩余距离 `0` 对 `15.54`。从 `(5, 5)` 走到 `(12, 12)` 只要 14 步。然后它把位置换成 RSSM latent，做了一次 5 步想象和一次 Actor 更新。参数变了，策略没有被证明变好。

这些数字属于 4.7，不必在本节里再跑一遍当主结果。本节要回答的是：**把数据、更新次数和闭环都加大之后，latent 策略有没有在真实环境里变好？如果没有，失败稳定地出在哪一环？**

<div style="text-align:center; margin:20px 0;">
  <img src="/carracing/dreamer-lite.png" alt="Dreamer-lite 完整训练循环" style="max-width:min(800px, 100%); height:auto; border:1px solid var(--vp-c-divider); border-radius:8px;">
  <div style="font-size:0.9em; color:var(--vp-c-text-2); margin-top:8px;">完整循环就这四步：收集像素、在 RSSM 里做梦、用梦里的回报更新 Actor-Critic、再回到环境。两份跟做 Notebook 只把接口接通；这里要让这个圈真的转起来。</div>
</div>
...
- 至少一个基线：随机策略，或第二份 Notebook 那条位置模型 MPC（`planned 1.0` / `random 0.0`）
- **真实环境 return 对真实交互步数**，不是对训练步数
- 同一起点只换动作的反事实
- 一组 Actor 或 Planner 利用模型漏洞的失败，固定 seed 可重复
- GPU / CPU、peak 显存、墙钟时间和 checkpoint sha256

下面先用仓库里能在 CPU 上跑完的一档（20 段 × 16 步，40 次世界模型更新）把数字钉死。本节必须比 smoke 更大，可以比这一档更大，但不能用第一份 Notebook 的 4 段、15 步冒充完整训练。

## 为什么本节是决策与规划路线的小整机

决策与规划路线的叙事是：VAE 的 latent 不够稳，RSSM 用 prior / posterior 双路替代；CMA-ES 没有梯度，Actor-Critic 用反向传播替代。两份跟做 Notebook 确认这些替代在接口上可行。本节要确认它们在训练上是否可行。
...
先用一段话回答：两份跟做 Notebook 已经证明了什么，还缺什么。可以引用 4.7 的数字，不要把那两份 Notebook 再抄一遍。
...
**运行这一步，你会看到什么？** 一段不超过半页的对照表。如果这一段写成「我复现了第一份 Notebook」，本节从这里就偏了。

**这就是衔接而不是重复**：4.7 的结论是本节的前提，不是本节的正文。

**一个值得做的实验**：把第二份 Notebook 里位置模型的训练起点收成左上角 3×3（45 条转移）。在我们跑过的这一次里，测试集四个起点仍然全部成功——残差 MLP 把「每步最多一格」写进了结构，短距离平移很容易外推。所以「缩小覆盖就一定掉点」不是自动成立的。你要找的失败，往往在 RSSM 和闭环里，不在这条可解释基线上。

## 第二步：环境依赖与数据卡

决策与规划路线需要 PyTorch。教学版 CPU 可跑；完整规模建议单张 24GB，并标明「24GB 目标」还是「CPU 缩减版」。
...
**一个值得做的实验**：用错误切法——把每段的前 8 步放 train、后 8 步放 test。多步指标会虚高，因为模型已经在同一条轨迹的前半段里见过方块怎么走。本节里必须写你没有这样做。
...
比第一份 Notebook 的 4 段 × 8 步更大，曲线仍然是同一句话：total 在降，重建还是赢不过复制上一帧。reward 头很快降到接近 0，因为它几乎只见过 `-0.01`。continue 从抛硬币的 `0.72` 降到 `0.23`，是因为每段最后一步 `done=True`，模型在学「16 步该结束了」，不是在学「碰到绿块该结束」。
...
**这就是第一份 Notebook 在更大一档上的重演**：数据加了五倍，更新加了一倍多，像素损失照样奖励偷懒。

**一个值得做的实验**：把 `num_episodes` 从 20 扫到 80，看重建第一次低于 `0.0057` 发生在哪一档。4.7 把这件事列为扩展；本节允许你把它做成主证据之一，但不能只交这一条。
...
for _ in range(50):          # smoke 只更新 1 次；这里至少几十次
...
```text
随机策略          真实 return 的下限
位置模型 MPC      可解释动态的上限（第二份 Notebook：planned 1.0 / random 0.0）
RSSM + Actor      你要交的曲线
```

`scripts/run_position_dynamics_reference.py --seed 0 --updates 100` 给出位置模型那一档：
...
字段、shape、seed、切分、时间对齐。wrapper 与两份跟做 Notebook 一致：`[B, T+1, H, W, 3]` 对 `[B, T]` 动作。
...
第二条命令是基线，不是本节本体。本体在你自己的 Notebook 里：收集、训练、想象、再收集。
...
- 「Actor 参数变了，所以策略变好了。」——那是第二份 Notebook 的结论，不是本节的结论。
- 「真实 return 没动，是因为 PixelWorld 不可控。」——位置模型已经 `1.0` 对 `0.0`。
- 「用第一份 Notebook 的 4 段曲线代替完整训练。」
- 「DMC 没跑，贴一张论文图。」
...
- **prior 不是 posterior**。第一份 Notebook 里 15 步后两者的 mean 距离仍有 `0.72`。梦从第一步就会偏。
...
至少提出两种解释，再选一项最小改动带到第 9 章或 9.4。

## 本节小结

- **本节是决策与规划路线的小整机**：从跟做实验扩到闭环，看「接口连通」和「策略收敛」之间的距离。
...
从第一份 Notebook 的 4 段到至少 20 段，从一次 Actor 更新到一条对交互步数的曲线——规模变化不是量变，是质变。你会看见 Dreamer 的承诺和代价：在想象中训练是可能的，但想象不是现实。

## 后续工作

本节用连续高斯 \(z_t\) 和 REINFORCE，已经够你看见闭环怎么裂开。后面三条常见的下一步，各自对应一种你可能刚刚写下来的失败：
...
1. Hafner, D., et al. (2019). Learning Latent Dynamics for Planning from Pixels. _ICML 2019_. [arXiv:1811.04551](https://arxiv.org/abs/1811.04551) —— PlaNet：RSSM + CEM，第二份 Notebook 里 beam search 的连续动作亲戚。
...
4. Hafner, D., et al. (2023). Mastering Diverse Domains through World Models. _arXiv:2301.04104_. [链接](https://arxiv.org/abs/2301.04104) —— DreamerV3：一套超参打通 150+ 任务，是 24GB 目标对照的工程标杆，不是本节的及格线。
