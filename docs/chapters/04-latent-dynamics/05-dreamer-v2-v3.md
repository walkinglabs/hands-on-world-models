...
Dreamer 用想象训练可微的 actor，适合连续控制。下一篇看 MuZero 怎样换一条路：不重建观察，而是用树搜索改进 policy。动手实验见 [4.7 动手：Dreamer 的简化实现](/chapters/04-latent-dynamics/08-dreamer-concise) 的第二份 Notebook「在想象中行动」。
