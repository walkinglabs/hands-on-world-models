...
在探讨 MuZero 之前，我们有必要简要回顾深度强化学习在规划（Planning）领域的发展脉络。早在 2016 年，AlphaGo [[Silver et al., 2016]](https://doi.org/10.1038/nature16961) 的横空出世证明了深度神经网络结合蒙特卡洛树搜索（MCTS）可以攻克被视为人工智能领域难以跨越的围棋难题。随后，AlphaGo Zero [[Silver et al., 2017]](https://doi.org/10.1038/nature24270) 和 AlphaZero [[Silver et al., 2018]](https://doi.org/10.1126/science.aar6404) 进一步移除了对人类专家数据的依赖，通过完全的自我博弈（Self-play）在国际象棋、将棋和围棋等多种完美信息博弈中达到了超人类的水平。
...
上述该公式中隐藏着一个极其尖锐的数学问题：**在没有任何重建观测损失（Observation Reconstruction Loss）的约束下，网络如何保证隐状态 $s^k$ 不会发生模式崩溃（Mode Collapse），即退化为全零或无意义的噪声矩阵？**
