...
## 潜在空间动力学：为什么我们需要它？
:label:subsec_latent_dynamics_motivation
...
## 变分目标函数：优化 RSSM
:label:subsec_variational_bound_rssm
...
3. **在脑海中模拟**：对于每一条采样的动作序列，利用 RSSM 的**先验模型**（方程 :eqref:eq_rssm_deter 和 :eqref:eq_rssm_prior），自回归地推演未来的潜在状态 $s_{t+1}, \dots, s_{t+H}$。
...
1. 在公式 :eqref:eq_rssm_prior 和 :eqref:eq_rssm_post 中，为什么先验模型只依赖历史信息 $h_t$，而后验模型除了历史还需要当前观测 $o_t$？
   - *提示*：想象你在闭着眼睛走路（只靠历史感觉）和睁开眼睛走路（结合当前视觉）。在未来的“想象”阶段，你能获取到真实的 $o_t$ 吗？
2. 如果我们在变分下界方程 :eqref:eq_planet_elbo 中移除 KL 散度项，即令 $\beta = 0$，潜在动力学模型会发生什么灾难性的失效？
   - *提示*：考虑后验模型可以完美地从图像中编码信息，但先验模型（由于没有被逼迫去匹配后验）会产生怎样的行为。当依赖先验进行多步 CEM 规划时会发生什么？
3. 阅读代码 `RSSMCell.step_posterior`。在计算后验的均值和方差时，为什么不直接只传入观测特征 `obs_embed`，而是必须拼接上确定性历史状态 `rnn_hidden`？
   - *提示*：单幅图像（如一帧静态照片）包含足够的信息来推断物体运动的速度和加速度吗？
