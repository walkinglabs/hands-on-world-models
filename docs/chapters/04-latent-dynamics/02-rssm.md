...
这一问题在 PlaNet [Hafner et al., 2019] 及其后续的 Dreamer [Hafner et al., 2020] 系列论文中得到了突破性的解决。这些工作提出了循环状态空间模型（Recurrent State Space Model, RSSM）。RSSM 优雅地结合了确定性循环神经网络（RNN）的高效记忆能力与随机状态空间模型（SSM）对环境不确定性的建模能力。在本节中，我们将从最基础的概率法则出发，严格推导 RSSM 的数学机制，并将其一步步转化为可运行的代码。

## 历史背景与经典动态模型的局限性
:label:sec_rssm_limitations
...
## 变分下界与目标函数推导
:label:sec_rssm_elbo_derivation
...
$$
:eqlabel:eq_marginal_likelihood
...
## 小结
:label:sec_rssm_summary

在本节中，我们回顾了状态空间模型的演进过程，从高中概率乘法法则出发推导出序列隐状态的联合概率，并深入推导了 RSSM 的确定性与随机性双流网络架构。我们详细解释了最大化证据下界（ELBO）在优化整个序列推断模型中的核心作用，并给出了结构严密的 PyTorch 实现。通过引入 $h_t$ 和 $s_t$ 的解耦设计，RSSM 不仅能够利用 RNN 提供长达数百步的长期跨度记忆，同时还保留了对下一步高方差环境的敏锐建模能力。

## 练习
:label:sec_rssm_exercises

1. 在 `forward_posterior` 中，为什么我们要从后验分布中采样 $s_t$ 并将其传递给下一个时间步作为历史，而不是从先验分布中采样？
   *提示：思考变分下界中重构损失 $\mathbb{E}_q[\log p(o_t \mid s_t)]$ 的计算，如果后验状态不参与序列的整体时间轴循环，会对随后的时间步反向传播产生怎样的影响？*
2. 试推导一下当时间步 $T \to \infty$ 时，如果 GRU 内部参数不当导致梯度消失，先验分布与后验分布将面临怎样的退化情况？
3. 动手修改代码：如果要求随机状态 $s_t$ 不是服从多元高斯分布，而是服从分类分布并使用 Gumbel-Softmax 重参数化技巧（正如后续 DreamerV2 [Hafner et al., 2021] 论文中所改进的那样），你需要更改模型中的哪些部分？
   *提示：你需要更改 `_build_dist` 中的 `Normal` 类及后续的 `rsample()` 采样逻辑。同时离散分布下的 KL 散度计算公式也会发生本质改变。*
