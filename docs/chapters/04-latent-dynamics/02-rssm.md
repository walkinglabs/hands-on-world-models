...
状态能预测了，下一步是让它替我们选动作。下一篇看 PlaNet 怎样在这个 latent 世界里用 CEM 现场搜索。动手训练见 [4.7 动手：Dreamer 的简化实现](/chapters/04-latent-dynamics/08-dreamer-concise) 的第一份 Notebook「学出一个潜在世界」。
