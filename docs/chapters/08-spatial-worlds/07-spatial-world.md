...
7.9 的手还在桌子上走。现在世界变成一张深度图、一块会动的俯视占用。
...
NeRF、3DGS、Mesh 是三种空间表示：连续场平滑但慢，高斯快，三角形方便碰撞。它们都不自动包含动作动态。四维世界分支交出的是两个最小接口——静态场和动作条件动态场。8.8 再补相机射线、体渲染和多视角。
...
完成后进入 [8.8 动手：空间世界二选一](/chapters/08-spatial-worlds/08-spatial-world)。
