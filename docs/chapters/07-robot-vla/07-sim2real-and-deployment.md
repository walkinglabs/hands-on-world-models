...
纯仿真 RL 便宜但不诚实；纯真机 RL 诚实但太贵。**HIL-SERL** 一类方法让人在真机上介入：策略在跑，人随时接管并留下那一段示范，再用这些数据做 off-policy 更新。它改的是**数据从哪来**，不是世界模型的接口。和 [7.3](/chapters/07-robot-vla/03-vision-language-action) 数据引擎的差别是：人给的是真机标签，生成模型给的是合成标签。两者都可以进同一 replay。课程不要求复现 HIL-SERL；若真机选题用了介入，必须在数据卡里写清介入比例和谁在何时接管。
...
[上一篇 7.6　物理引擎与仿真构建](/chapters/07-robot-vla/06-physics-and-simulation) · [下一篇 → 7.8　动手：扩散策略的从零开始实现](/chapters/07-robot-vla/08-data-to-generative-policy)
