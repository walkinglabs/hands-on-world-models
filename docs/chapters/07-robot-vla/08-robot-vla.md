...
> **前置知识**：你已经读过 7.1–7.7，知道机器人接口、生成式策略、VLA、世界模型检查器与 Sim-to-Real。这一节把它们真跑一遍。
...
这一节规模打折，原理不打折。先搭一台会听指令的 Tiny VLA，再在动作执行前加一个后果检查器。空间几何和驾驶占用留给 [8.7](/chapters/08-spatial-worlds/07-spatial-world)。
...
- **第一份 Notebook**：state BC \(0.498\rightarrow 0.377\)，VLA chunk \(0.525\rightarrow 0.277\)，换指令差 0.120，闭环成功率 0.188
- **第二份 Notebook**：outcome \(0.977\rightarrow 0.538\)，直达碰撞 1.000，重排 0.328，进展 \(-0.036\)
...
- **第一份 Notebook 的 chunk 三步 MSE 几乎一样。** 不要把「chunk 越长误差越大」写成已经观察到的事实。
- **第二份 Notebook 的碰撞头明显弱于状态头。** 总损失 0.538 里，状态 0.013、碰撞 0.525。单点样例选错直达，是这个不平衡的直接后果。
...
从 4.6 的赛车到这一节的桌面，世界模型的身体在变，那句话没有变：**在行动之前，先在内部预见行动的后果。**
...
第一份 Notebook 的 chunk 损失降了，成功率仍是 0.188。专家在自己的状态分布上做示范，模型一旦走偏，下一帧就越出训练支持。**ACT**（Zhao et al., 2023）用 Transformer 一次出一长段 chunk；**Diffusion Policy**（Chi et al., 2023）把动作当成要去噪的轨迹，能表达多峰。两者都还是模仿，都还需要第二份 Notebook 里那样的后果检查。
...
第二份 Notebook 只预测一步，碰撞头还没学干净。多步 rollout 会把 0.013 的状态误差滚成错误的碰撞判断。只报碰撞、不报进展，会得到一个学会刹车的模型。

## 参考文献

1. Brohan, A., et al. (2023). RT-2: Vision-Language-Action Models Transfer Web Knowledge to Robotic Control. _CoRL 2023_. [arXiv:2307.15818](https://arxiv.org/abs/2307.15818) —— 把视觉语言模型的知识迁到机器人动作。
2. Kim, M. J., et al. (2024). OpenVLA: An Open-Source Vision-Language-Action Model. [arXiv:2406.09246](https://arxiv.org/abs/2406.09246) —— 开源 VLA 基线，第一份 Notebook 的对照对象。
3. Zhao, T. Z., et al. (2023). Learning Fine-Grained Bimanual Manipulation with Low-Cost Hardware. _RSS 2023_. [arXiv:2304.13705](https://arxiv.org/abs/2304.13705) —— ACT：用 action chunk 做行为克隆。
4. Chi, C., et al. (2023). Diffusion Policy: Visuomotor Policy Learning via Action Diffusion. _RSS 2023_. [arXiv:2303.04137](https://arxiv.org/abs/2303.04137) —— 用扩散表达多峰动作，避免左右绕行被平均掉。
