...
| **零样本策略** | 点轨迹、未来视频或动作 token | 直接当策略，或把预测点当成要跟踪的目标 | Track2Act、World Action Models |

三种用法的反事实检查是同一句：固定当前观察，只换候选动作，预测必须分开。分不开，就还不是世界模型，只是视频生成或行为克隆。

下面把检查器写成可动手的最小系统。数据引擎和零样本策略在本节后半对照，配方仍走 [7.8](/chapters/07-robot-vla/08-robot-vla) 与 [7.12](/chapters/07-robot-vla/12-vla-checker)。
