...
完整的配方级实验在 [7.6 动手：扩散策略的从零开始实现](/chapters/07-robot-vla/06-data-to-generative-policy)；把检查器做成可验证证据，是 [7.8 动手：把世界模型接上身体](/chapters/07-robot-vla/08-world-model-meets-body) 的毕业设计。这里只给最小入口：两份教学 Notebook，CPU 上几分钟跑完。
...
[上一篇 7.2　行为克隆与扩散策略](/chapters/07-robot-vla/02-imitation-and-policies) · [下一篇 → 7.4　接触力与全身控制](/chapters/07-robot-vla/04-contact-and-whole-body)
