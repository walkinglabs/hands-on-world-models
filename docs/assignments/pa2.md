...
PA0 让你亲手发现「我的模型缺什么」。PA1 让你训练一台完整的世界模型，亲眼看到它在哪里崩溃。8.6 用六项测试把失败写成可复现的条件。
...
PA0–PA1 让你理解世界模型「是什么」和「怎么做」。8.6 让你学会「怎样审问」。PA2 让你做研究的最小完整循环：
...
对照实验应复用你 PA1 的训练脚本和 8.6 的评价函数，不要另写一套「专属指标」。

## 第一步：固定一个稳定失败

从 PA0 / PA1 / 8.6 里选一个**稳定出现**的失败。给出复现命令、数据、seed 和量化条件。
...
评价函数必须和 8.6 / PA1 用同一份。给新模型写专属指标，该项零分。
