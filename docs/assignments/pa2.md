...
>
> **前置知识**：你已经跑过 PA0、至少一条 PA1，以及 [8.5 / 8.7](/chapters/08-evaluate-and-invent/08-07-test-a-world-model)。PA2 从那里找到的稳定失败出发，只改一件事。第 8.6 节讲「为什么要这样设计实验」；本页是要交的作业。

---

PA0 让你亲手发现「我的模型缺什么」。PA1 让你训练一台完整的世界模型，亲眼看到它在哪里崩溃。8.7 用六项测试把失败写成可复现的条件。

现在你有一组稳定失败。PA2 的任务是：**完成一次可以被推翻的小型研究循环**。

不是提出「CNN + Transformer + Diffusion + MCTS」的宏大新架构。是问一个更小的问题：**如果只改一件事，能不能让这组失败消失？**

## 为什么 PA2 是整门课的终点

PA0–PA1 让你理解世界模型「是什么」和「怎么做」。8.7 让你学会「怎样审问」。PA2 让你做研究的最小完整循环：
...
模板在 `notebooks/assignments/PA2-next-model-template.ipynb`。单元格里的 `assert True` 是占位，不是完成标志。
...
对照实验应复用你 PA1 的训练脚本和 8.7 的评价函数，不要另写一套「专属指标」。

## 第一步：固定一个稳定失败

从 PA0 / PA1 / 8.7 里选一个**稳定出现**的失败。给出复现命令、数据、seed 和量化条件。
...
如果两种解释预测同一个 \(\Delta y\)，你无法区分它们。第 8.6 节把「四种结果怎么判」写成了图；本页要求你在跑实验之前把那张表填进报告。
...
评价函数必须和 8.7 / PA1 用同一份。给新模型写专属指标，该项零分。
...
- **不要提出宏大新架构。** PA2 的目标是走完研究循环，不是发明 DreamerV4。
