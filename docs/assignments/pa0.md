...
<div style="text-align:center; margin:20px 0;">
  ...
  <div style="font-size:0.9em; color:var(--vp-c-text-2); margin-top:8px;">一张还算干净的 5×5 世界：左边是乱走收集到的轨迹，中间是从轨迹里数出来的转移，右边是 MPC 走出来的路。PA0 要你打破的，正是这种「学到的和真的可以完全一致」。</div>
</div>
...
下面用仓库里的 5×5 网格把整条证据链先走一遍。你的作业可以换环境，但字段、切分、四类评价和稳定失败不能缺。
...
**一个值得做的实验**：先让你的变化「几乎不造成影响」——部分可观测时给很大的感受野，未覆盖区域先只挡住一格，规划深度先设到能看见终点。确认基线能通过。然后逐步收紧，直到基线稳定失败。PA0 的目的不是制造最复杂环境，而是让一种失败稳定出现。

## 第二步：自己收集数据，写数据卡

PA0 不给你数据集。`EmpiricalDynamics` 吃的是 `Transition`，不是随便三个元组。
...
若你把陷阱挪到贪心路上，例如 `traps=((0, 1), (1, 1))`，贪心第一步就会掉进 `(0, 1)`，回报变成 `-10`。那是另一种合法的 PA0：让无模型基线稳定失败，再看学到的模型能不能绕开。一次只选一种。
...
PA0 不要求你修好这个失败。它要求你**说清楚失败在哪里、为什么会出现、哪种解释已经被你排除**。
...
- **环境必须足够小**。PA0 的目标是让一种失败稳定出现。如果需要 GPU 或超过 5 分钟，说明设计太复杂了。
...
- **PA0 是整门课的分水岭**：从人工写的转移表到自行设计的 learned dynamics。
