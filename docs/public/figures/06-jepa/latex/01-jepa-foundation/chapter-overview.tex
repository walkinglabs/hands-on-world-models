...
\node[anchor=west, text=Ink] at (1,9.20) {\fontsize{18}{21}\selectfont\bfseries 第 6 章 · 联合嵌入预测架构};
\node[anchor=west, text=Slate] at (1,8.55) {\fontsize{9.2}{11}\selectfont 从非生成式特征预测，到防坍塌、动量自举与特征空间规划};
\draw[Brand!18, line width=0.7pt] (1,8.13) -- (24,8.13);
\node[card, fill=col0fill, draw=col0stroke, text width=7.06cm] (s1) at (4.720,6.35) {%
  {\fontsize{8.3}{10}\selectfont\bfseries\color{col0text} 6.1\quad 联合嵌入与特征预测}\par\vspace{2pt}
  {\fontsize{6.6}{8.299999999999999}\selectfont\color{Slate} I-JEPA 抛弃像素重构}
};
\node[card, fill=col1fill, draw=col1stroke, text width=7.06cm] (s2) at (12.500,6.35) {%
  {\fontsize{8.3}{10}\selectfont\bfseries\color{col1text} 6.2\quad 掩码机制与表示坍塌}\par\vspace{2pt}
  {\fontsize{6.6}{8.299999999999999}\selectfont\color{Slate} 方差—协方差撑开表示空间}
};
\node[card, fill=col2fill, draw=col2stroke, text width=7.06cm] (s3) at (20.280,6.35) {%
  {\fontsize{8.3}{10}\selectfont\bfseries\color{col2text} 6.3\quad 目标网络}\par\vspace{2pt}
  {\fontsize{6.6}{8.299999999999999}\selectfont\color{Slate} EMA 动量自举与跨 Patch 预测}
};
\node[card, fill=col3fill, draw=col3stroke, text width=7.06cm] (s4) at (20.280,2.55) {%
  {\fontsize{8.3}{10}\selectfont\bfseries\color{col3text} 6.4\quad 动作条件特征预测}\par\vspace{2pt}
  {\fontsize{6.6}{8.299999999999999}\selectfont\color{Slate} Action-JEPA 时空动力学}
};
\node[card, fill=col4fill, draw=col4stroke, text width=7.06cm] (s5) at (12.500,2.55) {%
  {\fontsize{8.3}{10}\selectfont\bfseries\color{col4text} 6.5\quad JEPA 从零实现}\par\vspace{2pt}
  {\fontsize{6.6}{8.299999999999999}\selectfont\color{Slate} Gather 索引与动量自举闭环}
};
\node[card, fill=col5fill, draw=col5stroke, text width=7.06cm] (s6) at (4.720,2.55) {%
  {\fontsize{8.3}{10}\selectfont\bfseries\color{col5text} 6.6\quad 视频 JEPA 简洁实现}\par\vspace{2pt}
  {\fontsize{6.6}{8.299999999999999}\selectfont\color{Slate} 特征空间可微 MPC 轨迹优化}
};
