...
\node[anchor=west, text=Ink] at (1,9.20) {\fontsize{18}{21}\selectfont\bfseries 第 2 章 · 深度学习与表征基础};
\node[anchor=west, text=Slate] at (1,8.55) {\fontsize{9.2}{11}\selectfont 从空间特征与时序记忆，到隐空间生成与标准化实现};
\draw[Brand!18, line width=0.7pt] (1,8.13) -- (24,8.13);
\node[card, fill=col0fill, draw=col0stroke, text width=7.06cm] (s1) at (4.720,6.35) {%
  {\fontsize{8.3}{10}\selectfont\bfseries\color{col0text} 2.1\quad CNN 与 ViT}\par\vspace{2pt}
  {\fontsize{6.6}{8.299999999999999}\selectfont\color{Slate} 局部归纳偏置与全局注意力}
};
\node[card, fill=col1fill, draw=col1stroke, text width=7.06cm] (s2) at (12.500,6.35) {%
  {\fontsize{8.3}{10}\selectfont\bfseries\color{col1text} 2.2\quad RNN 与 Transformer}\par\vspace{2pt}
  {\fontsize{6.6}{8.299999999999999}\selectfont\color{Slate} 循环记忆与因果时序建模}
};
\node[card, fill=col2fill, draw=col2stroke, text width=7.06cm] (s3) at (20.280,6.35) {%
  {\fontsize{8.3}{10}\selectfont\bfseries\color{col2text} 2.3\quad VAE 与 VQ}\par\vspace{2pt}
  {\fontsize{6.6}{8.299999999999999}\selectfont\color{Slate} 连续隐变量与离散码本}
};
\node[card, fill=col3fill, draw=col3stroke, text width=7.06cm] (s4) at (20.280,2.55) {%
  {\fontsize{8.3}{10}\selectfont\bfseries\color{col3text} 2.4\quad 自回归与扩散}\par\vspace{2pt}
  {\fontsize{6.6}{8.299999999999999}\selectfont\color{Slate} 次词元预测与逆向去噪}
};
\node[card, fill=col4fill, draw=col4stroke, text width=7.06cm] (s5) at (12.500,2.55) {%
  {\fontsize{8.3}{10}\selectfont\bfseries\color{col4text} 2.5\quad 基础组件从零实现}\par\vspace{2pt}
  {\fontsize{6.6}{8.299999999999999}\selectfont\color{Slate} 纯张量前向与反向闭环}
};
\node[card, fill=col5fill, draw=col5stroke, text width=7.06cm] (s6) at (4.720,2.55) {%
  {\fontsize{8.3}{10}\selectfont\bfseries\color{col5text} 2.6\quad 基础组件简洁实现}\par\vspace{2pt}
  {\fontsize{6.6}{8.299999999999999}\selectfont\color{Slate} 归一化与残差骨干统一接口}
};
