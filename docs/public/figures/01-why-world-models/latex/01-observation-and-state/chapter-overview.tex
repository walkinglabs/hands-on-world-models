...
\node[anchor=west, text=Ink] at (1,9.20) {\fontsize{18}{21}\selectfont\bfseries 第 1 章 · 为什么需要世界模型};
\node[anchor=west, text=Slate] at (1,8.55) {\fontsize{9.2}{11}\selectfont 从不完备观测，到可交互的预测与验证闭环};
\draw[Brand!18, line width=0.7pt] (1,8.13) -- (24,8.13);
\node[card, fill=col0fill, draw=col0stroke, text width=5.12cm] (s1) at (3.748,4.75) {%
  {\fontsize{8.3}{10}\selectfont\bfseries\color{col0text} 1.1\quad 观察、状态与变化}\par\vspace{2pt}
  {\fontsize{6.6}{8.299999999999999}\selectfont\color{Slate} 历史聚合为马尔可夫状态}
};
\node[card, fill=col1fill, draw=col1stroke, text width=5.12cm] (s2) at (9.582,4.75) {%
  {\fontsize{8.3}{10}\selectfont\bfseries\color{col1text} 1.2\quad 什么是世界模型}\par\vspace{2pt}
  {\fontsize{6.6}{8.299999999999999}\selectfont\color{Slate} 转移、奖励与规则形式化}
};
\node[card, fill=col2fill, draw=col2stroke, text width=5.12cm] (s3) at (15.418,4.75) {%
  {\fontsize{8.3}{10}\selectfont\bfseries\color{col2text} 1.3\quad 经典世界模型}\par\vspace{2pt}
  {\fontsize{6.6}{8.299999999999999}\selectfont\color{Slate} 从控制论到神经世界模型}
};
\node[card, fill=col3fill, draw=col3stroke, text width=5.12cm] (s4) at (21.252,4.75) {%
  {\fontsize{8.3}{10}\selectfont\bfseries\color{col3text} 1.4\quad 交互式驾驶}\par\vspace{2pt}
  {\fontsize{6.6}{8.299999999999999}\selectfont\color{Slate} 观察—预测—选择—验证}
};
