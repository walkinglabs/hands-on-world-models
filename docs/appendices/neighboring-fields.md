...
| 行为克隆 | $\pi(a\mid o)$ 拟合专家 | 回报与转移 | [7.2](/chapters/07-robot-vla/02-imitation-and-policies) |
...
当环境里还有其他学习主体时，转移不再是平稳的 $P(s'\mid s,a)$：别人的策略也在变。这就是 **MARL** 和非平稳。自对弈（AlphaZero、Hide and Seek）把对手当作环境的一部分一起学。

对世界模型的直接后果：你学到的动力学里混进了别人当时的策略。换一个对手，同一动作的后果会变。诚实的做法是把其他主体的动作也写进条件，

$$
\hat s_{t+1}=f_\theta(s_t, a_t^{\text{self}}, a_t^{\text{others}}),
$$

而不是假装世界只有自己。本书实验默认单智能体；若做对抗或协作，必须把「谁的动作进了条件」写进数据卡。
