...
- **2018 年**，David Ha 与 Schmidhuber 发表 **World Models**：VAE 压缩画面、MDN-RNN 预测未来、一个 867 个参数的小控制器完全在"梦境"中训练，再迁回真实环境。V-M-C 框架与交互式的展示方式让它成为公认的现代起点——本课程第 1 章的原型正是这篇论文，[4.6 动手：World Models 的复现](/chapters/04-decision-and-planning/06-reproduce-world-models) 用 CarRacing 把它完整跑通。
