...
class NotebookSmokeTest(unittest.TestCase):
    ...
    def test_routes_bc_notebooks_run(self):
        paths = [
            "notebooks/03_interactive_video/B1-compress-and-predict-video.ipynb",
            "notebooks/03_interactive_video/B2-make-video-controllable.ipynb",
            "notebooks/04_jepa/C1-learn-video-features.ipynb",
            "notebooks/04_jepa/C2-test-and-control-features.ipynb",
        ]
        for path in paths:
            with self.subTest(path=path):
                self.execute_notebook(path)
    ...
    def test_routes_de_notebooks_run(self):
        paths = [
            "notebooks/05_robot/D1-build-a-tiny-vla.ipynb",
            "notebooks/05_robot/D2-check-actions-before-moving.ipynb",
            "notebooks/06_spatial/E1-from-camera-to-space.ipynb",
            "notebooks/06_spatial/E2a-build-a-small-4d-world.ipynb",
            "notebooks/06_spatial/E2b-predict-driving-space.ipynb",
        ]
        for path in paths:
            with self.subTest(path=path):
                self.execute_notebook(path)
